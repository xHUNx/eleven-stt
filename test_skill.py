...
def validate_skill_manifest(skill_dir):
    ...
    try:
        skill_info = yaml.safe_load(yaml_content)
    except yaml.YAMLError as e:
        print(f"❌ Invalid YAML in SKILL.md: {e}")
        return False
    ...
    if missing_fields:
        print(f"❌ Missing required fields in SKILL.md: {missing_fields}")
        return False
    
    print(f"✅ Skill manifest valid: {skill_info['name']} v{skill_info['version']}")
    return True
